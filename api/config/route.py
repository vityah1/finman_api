...
from flask_cors import request, cross_origin
...
@config_bp.route("/api/users/<user_id>/config", methods=["GET"])
@cross_origin()
@jwt_required()
def get_configs(user_id):
    ...
    get configs
    """
    return get_configs_(user_id)


@config_bp.route("/api/users/<user_id>/config", methods=["POST"])
@cross_origin()
@jwt_required()
